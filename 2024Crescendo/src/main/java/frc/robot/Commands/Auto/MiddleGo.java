...
public class MiddleGo extends SequentialCommandGroup{
    public MiddleGo(){
        ...
        addCommands(
           new InstantCommand( ()->{
            if(DriverStation.getAlliance().get()==Alliance.Red)
                {
                 Pose2d _Pose=PathPlannerPath.fromPathFile("MiddleGo").flipPath().getPreviewStartingHolonomicPose();
                
                 RobotContainer.m_swerve.ResetOdometry(_Pose);
                }else 
                {
                    RobotContainer.m_swerve.ResetOdometry(PathPlannerPath.fromPathFile("MiddleGo").getPreviewStartingHolonomicPose());
                }
            }
            ...
            new WaitCommand(7.),
            RobotContainer.m_swerve.followPathCommand("Middle",false).raceWith(new NoteIntake(Button.kAutoButton.value))
